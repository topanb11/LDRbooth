...
/**
 * Redraws the final strip using whatever theme is currently selected.
 * Safe to call repeatedly (e.g. every time the theme dropdown changes) —
 * it only touches the download link once every photo has been redrawn.
 */
export function renderFinalComposite(): void {
  ...
  if (theme.imagePath) {
    ...
    bg.onerror = () => {
      // Fall back to plain white if the theme asset fails to load.
      ctx.fillStyle = "#ffffff";
      ctx.fillRect(0, 0, width, height);
      drawForeground();
    };
    bg.src = theme.imagePath;
  } else {
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(0, 0, width, height);
    drawForeground();
...
function drawTitle(ctx: CanvasRenderingContext2D, width: number): void {
  ...
  ctx.fillText("LDRbooth", width / 2, PADDING + 32);
}
...
// Draw an image into a target rect using "cover" semantics (crop to fill).
function drawCover(
